...
class _JobPostCardState extends State<JobPostCard> {
  ...
  Future<void> _checkApplicationStatus() async {
    try {
      ...
      setState(() {
        _isApplied = applicationSnapshot.docs.isNotEmpty;
      });
    } catch (e) {
  ...
  Future<void> _toggleJobApplication() async {
    try {
      ...
      if (currentUser.uid == widget.job.employerId) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Kendi iş ilanınıza başvuramazsınız')),
        );
        return;
      }

      final jobApplicationsRef =
          FirebaseFirestore.instance.collection('job_applications');

      if (_isApplied) {
        // Remove application
        final querySnapshot = await jobApplicationsRef
            .where('jobId', isEqualTo: widget.job.id)
            .where('applicantId', isEqualTo: currentUser.uid)
            .get();
  ...
  Future<void> _sendAutomaticMessage(String applicantId) async {
    ...
    try {
      ...
      final automaticMessage = 'Merhaba, Ben $senderName. Paylaştığınız ${widget.job.jobTitle} işi hakkında daha detaylı bilgi alabilir miyim?';
   ...
   try {
        final chatDocRef = FirebaseFirestore.instance
          .collection('chats')
          .doc(generateChatId(FirebaseAuth.instance.currentUser!.uid, widget.job.employerId));
         if(_messageId != null){
            ...
            const SizedBox(height: 10),
             if (!isJobOwner)
              Row(
                children: [
                   Expanded(
                      child:  FirebaseAuth.instance.currentUser != null ? ElevatedButton(
                         onPressed: _toggleJobApplication,
                        ...
                        child: Text(_isApplied ? 'Başvuruyu Geri Çek' : 'Başvur'),
                          ) : ElevatedButton(
                                   onPressed: () {
                                      ScaffoldMessenger.of(context).showSnackBar(
                                        const SnackBar(
                              ...
                              style: ElevatedButton.styleFrom(
                              foregroundColor: Colors.white,
                              backgroundColor: Colors.grey, // or some other color that indicates disabled state
                             minimumSize: const Size(double.infinity, 45),
                           shape: RoundedRectangleBorder(
                            borderRadius: BorderRadius.circular(10),
                            ),
                          ...
                          // Firestore'dan mesajı göndereceğimiz kullanıcının verilerini alıyoruz
                           DocumentSnapshot userDoc = await FirebaseFirestore.instance
                            ...
                            final userData = userDoc.data() as Map<String, dynamic>;
                             ...
                             final currentUserProfileImageUrl = currentUser.photoURL ?? '';
