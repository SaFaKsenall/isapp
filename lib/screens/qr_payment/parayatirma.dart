...
Future<void> _fetchUserDataAndDailyDeposits() async {
  try {
    ...
    if (currentUser == null) {
      ...
      DocumentSnapshot balanceDoc = await _firestore.collection('balances')
          .doc(currentUser.uid)
          .get();
      
      setState(() {
        _currentBalance = balanceDoc.exists 
          ? (balanceDoc.data() as Map<String, dynamic>)['amount'] as double? ?? 0.0
          : 0.0;
      });
    ...
    DateTime endOfDay = startOfDay.add(const Duration(days: 1));
      ...
      QuerySnapshot dailyDeposits = await _firestore.collection('deposits')
      ...
      setState(() {
        _dailyTotalDeposit = dailyDeposits.docs.fold(0.0, 
          (total, doc) {
  ...
  void _depositMoney() async {
    ...
    if (amount < DepositConfig.MIN_DEPOSIT_AMOUNT) {
      _showErrorSnackBar('En az ${DepositConfig.MIN_DEPOSIT_AMOUNT} TL yatırabilirsiniz');
      return;
    }

    if (amount > DepositConfig.MAX_SINGLE_DEPOSIT) {
      _showErrorSnackBar('Tek seferde maks ${DepositConfig.MAX_SINGLE_DEPOSIT} TL yatırabilirsiniz');
      return;
    }
    ...
    if (_dailyTotalDeposit + amount > DepositConfig.MAX_DAILY_DEPOSIT) {
      _showErrorSnackBar(
        'Günlük para yatırma limitini aştınız. '
