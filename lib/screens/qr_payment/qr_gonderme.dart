...
class _ReferralQRCodePageState extends State<ReferralQRCodePage> {
  ...
  Future<void> _handleScannedReferralCode(String scannedCode, int amount) async {
    try {
      User? currentUser = FirebaseAuth.instance.currentUser;
      ...
      DocumentReference receiverBalanceRef = FirebaseFirestore.instance
          .collection('balances')
          .doc(targetUserId);
      batch.set(receiverBalanceRef, {
        'uid': targetUserId,
        'amount': FieldValue.increment(amount),
        'updatedAt': FieldValue.serverTimestamp()
      ...
      DocumentReference transactionLogRef = FirebaseFirestore.instance
      ...
      body: Center(
        child: _isLoading
            ...
            : _referralCode.isEmpty
                ? const Text('Referans kodu bulunamadı')
                : Column(
